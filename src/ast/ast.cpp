...
#include <iostream> // use std::string
...
void DeclNode::AddDecl() {
  ...
  array_dimension_list_list_.push_back(new_list); // the new std::list is empty by default
  auto itr = array_dimension_list_list_.rbegin(); // the new std::list<Node*>
  for (auto itr1 : tmp_array_dimension_array_) {
    assert(*itr != nullptr);
    assert(itr1 != nullptr);
    (*itr)->push_back(itr1); // use tmp_array_dimension_array_ to fill the new std::list<Node*>
  }
...
// static member
std::vector<Node*> DeclNode::tmp_array_dimension_array_; // empty by default
std::string DeclNode::tmp_ident_; // empty string
...
std::vector<Node*> tmp_fparam_index_list_; // empty by default
...
// static member
std::vector<Node*> tmp_index_array_; // empty by default